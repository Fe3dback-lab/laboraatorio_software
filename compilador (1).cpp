...
FILE *figuras; // archivo donde se guarda la representación intermedia

// Variables auxiliares para recolectar info de cada figura
char tipoFiguraActual[30];
char nombreFigura[30];
int parametros[10]; // hasta 10 parámetros por figura
...
void parseFiguraDecl() {
    // Detectar tipo de figura y guardar como texto
    ...
    // Guardar nombre de la figura
    strcpy(nombreFigura, lexema);
    match(IDENTIFICADOR);

    match(PARENTESIS_INI);

    // Leer y guardar parámetros numéricos
    ...
    while (tokenActual == COMA) {
        // Mirar lo que sigue después de la coma (sin consumirlo aún)
        int token_siguiente;
        ...
        if (tokenActual == NUMERO) {
            ...
            // Restaurar estado del archivo y lexema si lo que venía no era número
            fseek(tokens, pos, SEEK_SET);
            strcpy(lexema, lexema_copia);
            tokenActual = COMA; // volvemos al estado justo antes
            break; // dejamos de procesar parámetros
        } else {
            error("Se esperaba un número o un color después de la coma");
        }
    }

    // Ahora esperamos la coma final que precede al color
    match(COMA);
